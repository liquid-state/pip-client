...
export default class PIPForm implements IPIPForm {
  ...
  async form(): Promise<FormResponse> {
    ...
    const [schema, translations, data] = await Promise.all<
      FormSchema,
      object,
      FormData
    >([
  ...
  async submit(formData: object, extraData: object = {}) {
    ...
    const dataTypeKey = formType.children.find(
      child => child.indexOf("data") !== -1
    );
    if (!dataTypeKey) {
  ...
  private async getFormSchema(formType: ObjectType): Promise<FormSchema> {
    let form = await this.pip.getLatestObjectForType<RawFormSchema>(
      formType,
      this.jwt,
      true
    );
  ...
  private async getFormTranslations(childTypes: ObjectType[]): Promise<object> {
    ...
    let translations = await this.pip.getLatestObjectForType(
      type,
      this.jwt,
      true
    );
